...
class _ClinicMapScreenState extends State<ClinicMapScreen> {
  ...
  Future<void> _initializeMap() async {
    ...
    try {
      ...
      if (mounted) _showLocationFailedDialog(false);
    }
  }

  void _showLocationFailedDialog(bool permanentlyDenied) {
    showDialog(
      context: context,
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(context);
              final fallback = _locationService.getGuatemalaCityPosition();
              setState(() {
                _currentPosition = fallback;
                _isRealLocation = false;
                _errorMessage =
                    'Mostrando clínicas cerca de Guatemala City (sin GPS)';
              });
              _loadNearbyClinics();
            },
  ...
  Future<void> _loadNearbyClinics() async {
    ...
    try {
      List<DentalClinic> clinics =
          await _locationService.findNearbyDentalClinics(
          ...
          markers: [
            Marker(
              ...
              child: Container(
                ...
                child: Icon(
                  _isRealLocation ? Icons.person : Icons.location_city,
                  color: Colors.white,
                  size: 20,
                ),
              ),
