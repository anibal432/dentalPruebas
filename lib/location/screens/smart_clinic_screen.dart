...
// ─────────────────────────────────────────────────────────────────────────────
// CLÍNICAS AFILIADAS
// Pon aquí el doc ID de Firestore de tu clínica registrada.
// Aparecerá SIEMPRE en primer lugar, sin importar el diagnóstico.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Edita firestoreId con el ID real de tu documento en Firestore ────────────
const List<AffiliatedClinic> _affiliatedList = [
  AffiliatedClinic(
    firestoreId: 'REEMPLAZA_CON_DOC_ID', // ← pega aquí el ID de Firestore
...
//
// Las clínicas de Firestore (registered_clinics) reciben un boost de 999
// para garantizar que SIEMPRE aparezcan primero, sin importar qué
// diagnóstico se haya detectado.
// ─────────────────────────────────────────────────────────────────────────────
List<_ScoredClinic> _scoreAndSort(
  List<DentalClinic> clinics, {
  String? detectedCondition,
  Set<String>? firestoreIds,
}) {
  ...
  final scored = clinics.map((clinic) {
    ...
    // ── Boost máximo para clínicas registradas en nuestra BD ──────────────
    // 999 garantiza que siempre estén por encima de cualquier clínica de OSM
    ...
    if (isFS && detectedCondition != null) {
      // Menciona el diagnóstico y confirma que pueden tratarlo
      reason =
          '🦷 Puede tratar $detectedCondition · Clínica verificada';
    } else if (isFS) {
...
class _SmartClinicScreenState extends State<SmartClinicScreen>
  ...
  Future<void> _loadClinics() async {
    ...
    try {
      final raw = await _locationService.findNearbyDentalClinics(
      ...
      // IDs de Firestore: documentos cuyos IDs no son puramente numéricos
      final firestoreIds = raw
          .where((c) => !RegExp(r'^\d+$').hasMatch(c.id))
          .map((c) => c.id)
          .toSet();

      final scored = _scoreAndSort(
        raw,
        detectedCondition: widget.detectedCondition,
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Error al buscar clínicas: $e';
        _loading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton:
          !_loading && _scored.isNotEmpty ? _buildFAB() : null,
    );
  }
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      title: Column(
        ...
        children: [
          const Text('Clínicas Cercanas',
              style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
          if (widget.detectedCondition != null)
            Text(
              '🧠 Diagnóstico: ${widget.detectedCondition}',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(_error ?? 'Sin clínicas en el área',
                textAlign: TextAlign.center,
                style: const TextStyle(fontSize: 16, color: Colors.grey)),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              onPressed: () {
                setState(() => _radiusKm = (_radiusKm + 10).clamp(0, 50));
                _loadClinics();
              },
              icon: const Icon(Icons.zoom_out_map_rounded),
              label:
                  Text('Ampliar a ${(_radiusKm + 10).clamp(0, 50).toInt()} km'),
  ...
  Widget _buildContent() {
    ...
    return ListView(
      ...
      children: [
        ...
        _buildSectionHeader('⭐ Recomendado para ti', Colors.teal.shade700),
        const SizedBox(height: 8),
        _buildTopCard(top),
        if (rest.isNotEmpty) ...[
          const SizedBox(height: 24),
          _buildSectionHeader(
              'Otras clínicas cercanas (${rest.length})',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Recomendación personalizada',
                    style: TextStyle(
                        color: Colors.white,
                ...
                Text(
                  'Diagnóstico: ${widget.detectedCondition}. '
                  'Te mostramos clínicas que pueden atenderte.',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Sin GPS — mostrando clínicas cerca de Guatemala City',
  ...
  Widget _buildTopCard(_ScoredClinic sc) {
    ...
    final badgeColor = isFS ? Colors.teal.shade800 : Colors.blue.shade600;
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isFS ? Colors.teal.shade400 : Colors.teal.shade100,
            width: isFS ? 2 : 1,
          ),
          boxShadow: [
            BoxShadow(
              color: Colors.teal.withAlpha(isFS ? 60 : 20),
              blurRadius: isFS ? 24 : 8,
              offset: const Offset(0, 4),
            ),
          ],
        ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Text(
                          '${clinic.distanceInKm?.toStringAsFixed(1) ?? '?'}',
                          style: const TextStyle(
                              color: Colors.white,
                              fontSize: 14,
                              fontWeight: FontWeight.w800),
        ...
        children: [
          ...
          Expanded(
            child: Text(
              text,
              style: TextStyle(
                  color: textColor ?? Colors.grey.shade600, fontSize: 13),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
            ),
          ),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 32,
                  decoration: BoxDecoration(
                    color: isFS ? Colors.teal.shade100 : Colors.grey.shade100,
                    shape: BoxShape.circle,
                  ),
                  child: Center(
                    child: Text(
                      ...
                      if (clinic.openingHours != null) ...[
                        ...
                        Row(children: [
                          Icon(Icons.access_time_rounded,
                              size: 11, color: Colors.teal.shade300),
                          const SizedBox(width: 3),
                          Expanded(
                            child: Text(clinic.openingHours!,
                                style: TextStyle(
  ...
  void _openDetail(DentalClinic clinic) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            ClinicDetailScreen(clinic: clinic, userPosition: _position),
      ),
    );
  }
}
