...
class _DentalScanScreenState extends State<DentalScanScreen>
  ...
  bool _posibleBrackets = false;
  Map<String, dynamic>? _diagnosis;
  Interpreter? _interpreter;
  bool _modelLoaded = false;
  final ImageValidationService _imageValidator = ImageValidationService();
  ...
  // ← ACTUALIZADO: 6 clases (modelo v7), mismo orden que en Python
  ...
  // ← ACTUALIZADO: umbral para 'Otro' agregado
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      if (picked != null) {
        ...
        final esImagenValida = await _imageValidator.esImagenDental(file);
        ...
        if (!esImagenValida) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'La imagen no parece ser una boca o dentadura. '
                'Toma una foto más cercana de tus dientes.',
              ),
        ...
        // Heurística de brackets/objetos metálicos (no bloquea, solo advierte)
        _posibleBrackets = await _imageValidator.probablementeTieneBrackets(file);
      ...
      if (mounted) setState(() => _isValidatingImage = false);
    }
  }
  ...
  Future<void> _analyzeImage() async {
    ...
    try {
      ...
      // ← ACTUALIZADO: 'Otro' queda fuera del cálculo de patologías
      final top1EsPatologia = top1Label != 'Sano' && top1Label != 'Otro';
      final top2EsPatologia = top2Label != 'Sano' && top2Label != 'Otro';
      ...
      // ← NUEVO: bloque para 'Otro' (imagen no dental), va primero
      if (top1Label == 'Otro' && top1Score >= (_umbrales['Otro'] ?? 0.80)) {
        labelFinal = 'Imagen no válida';
        confianzaFinal = top1Score;
        esSano = false;
      } else if (top1Label == 'Sano' && top1Score >= umbral1) {
        labelFinal = 'Sano';
        confianzaFinal = top1Score;
        esSano = true;
      ...
      setState(() {
        _diagnosis = {
          ...
          'posibleBrackets': _posibleBrackets,
        };
        _isAnalyzing = false;
      });
  ...
  Color _colorPorLabel(String label) {
    ...
    // ← NUEVO
    if (l.contains('no válida') || l.contains('otro')) return const Color(0xFF607D8B);
  ...
  IconData _iconPorLabel(String label) {
    ...
    // ← NUEVO
    if (l.contains('no válida') || l.contains('otro')) return Icons.no_photography_outlined;
  ...
  String _descripcionPorLabel(String label) {
    ...
    // ← NUEVO
    if (l.contains('no válida')) {
      return 'La imagen no parece corresponder a una boca o dentadura. '
          'Por favor toma una foto clara de tus dientes.';
    }
  ...
  Widget _buildImagePreview() {
    return Stack(children: [
      ...
      if (_isAnalyzing || _isValidatingImage)
        Container(
          decoration: BoxDecoration(
  ...
  Widget _buildResultadoEnfermedad(Map<String, dynamic> diag) {
    ...
    for (int i = 0; i < _labels.length; i++) {
      // ← ACTUALIZADO: excluye también 'Otro' del listado de condiciones
      if (_labels[i] != 'Sano' && _labels[i] != 'Otro' && scores[i] > 0.05) {
  ...
  Widget _buildClinicButton(String detectedLabel) {
    // ← ACTUALIZADO: también oculta el botón para 'Imagen no válida'
    if (detectedLabel == 'Sano' ||
        detectedLabel == 'No concluyente' ||
        detectedLabel == 'Imagen no válida') {
  ...
  Widget _buildResults() {
    ...
    final posibleBrackets = (_diagnosis!['posibleBrackets'] as bool?) ?? false;
    ...
    return FadeTransition(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Advertencia de brackets/objetos metálicos ─
          if (posibleBrackets) ...[
            Container(
              margin: const EdgeInsets.symmetric(horizontal: 16),
              padding: const EdgeInsets.all(14),
              ...
              child: const Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Se detectaron posibles brackets, ortodoncia u objetos '
                      'metálicos en la imagen. Esto puede afectar la '
                      'precisión del diagnóstico. Si tienes ortodoncia, '
                      'toma este resultado como orientativo.',
                      style: TextStyle(
                        fontSize: 12,
